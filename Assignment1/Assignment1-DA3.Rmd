...
datasummary( w*factor(stfips) ~ N + SD + Percent() + Mean, data = df1 )
  ...
  # to simplify regions
  df1 <- df1[stfips %in% c("WA", "OR", "MT", "ID", "WY", "NV", "UT", "CO", "AZ", "NM", "HI", "AK", "CA"), region := "west"]
  df1 <- df1[stfips %in% c("ND", "SD", "NE", "KS", "MN", "IA", "MO", "WI", "IL", "IN", "MI", "OH"), region := "midwest"]
  df1 <- df1[stfips %in% c("OK", "TX", "AR", "LA", "KY", "TN", "MS", "AL", "WV", "VA", "NC", "SC", "GA", "FL", "DC","MD","DE"), region := "south"]
  df1 <- df1[stfips %in% c("PA", "NY", "VT", "NH", "ME","MA","RI","CT","NJ"), region := "north-east"]
...
df1[race == 1, white := 1]
df1[race >= 2, white := 0]
df1$white <- factor(df1$white)
df1 <- df1[, race := NULL]
  datasummary( w*factor(white) ~ N + SD + Percent() + Mean, data = df1 )
...
datasummary( w*factor(age) ~ N + Percent() + Mean, data = df1 )
  ggplot(df1, aes(x=age, y=w)) +
    geom_point() +
    geom_smooth(method="loess")
  # creating age square for quadritic distribution
...
## MARITAL ##
datasummary( w*factor(marital_status) ~ N +SD + Percent() + Mean, data = df1 )
...
df1[prcitshp == "Native, Born In US" | prcitshp == "Native, Born in PR or US Outlying Area" 
    | prcitshp == "Native, Born Abroad Of US Parent(s)" , native := 1] 
df1[prcitshp == "Foreign Born, US Cit By Naturalization" | prcitshp == "Foreign Born, Not a US Citizen" , native := 0]
...
# Not including in the model as there is no significant difference in wage
  datasummary( w*factor(private) ~ N + Percent() + Mean, data = df1 ) 
 df1[class == "Government - Federal" | class == "Government - Local" | class == "Government - State", private := 0] 
df1[class == "Private, For Profit" | class == "Private, Nonprofit" , private := 1]
df1 <- df1[,class := NULL]
